...
``deerflow.runtime.stream_bridge.memory.MemoryStreamBridge``.

Each run gets a per-``run_id`` event log protected by an
:class:`asyncio.Condition`.  Subscribers can replay from a
``last_event_id``; the buffer is bounded to *queue_maxsize*
entries.
"""
...
class _RunStream:
    ...
    Attributes:
        events: Ordered event log (oldest first).
        condition: Broadcasts to all subscribers when a new
            event arrives or the stream ends.
        ended: ``True`` after :meth:`publish_end` is called.
        start_offset: How many events have been evicted from
            the front of the buffer (for ``Last-Event-ID``
            resolution).
    """
...
class MemoryStreamBridge(StreamBridge):
    ...
    Events are retained for a bounded window per run so late
    subscribers and reconnecting clients can replay buffered
    events from ``Last-Event-ID``.

    Args:
        queue_maxsize: Maximum number of events retained per
            run.  When the buffer is full the oldest events
            are evicted.
    """
    ...
    def _resolve_start_offset(
        ...
        if stream.events:
            logger.warning(
                "last_event_id=%s not found in retained buffer; "
                "replaying from earliest retained event",
                last_event_id,
            )
        return stream.start_offset
        ...
        while True:
            async with stream.condition:
                if next_offset < stream.start_offset:
                    logger.warning(
                        "subscriber for run %s fell behind retained buffer; "
                        "resuming from offset %s",
                        run_id,
                        stream.start_offset,
                    )
